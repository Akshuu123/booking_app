...
const DestinationCard = ({ url, imgHeight, imgWidth, imgUrl, title, description }) => {
  url="/continent/europe/city/london"
  return (
    <div className="relative w-full max-w-sm sm:w-fit mx-auto">
      ...
      <Link href={url || "/"}>
        <img
          ...
          alt="image"
          className={`object-cover w-full duration-1000 hover:scale-105  ${imgHeight ?? 'h-[360px]'} ${imgWidth ?? 'sm:w-[278px]'}  rounded-lg`} // Full width inside the container
        />

        {/* Text Overlay */}
        <div className="absolute bottom-0 w-full p-6 rounded-b-lg">
          <h2 className="text-[24px] font-semibold text-white">{title || "3. China"}</h2>
          <p className="text-[16px] font-medium text-solitude">{description || "140 Activities"}</p>
        </div>
      </Link>
    </div>
  );

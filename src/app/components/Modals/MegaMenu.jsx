...
const  MegaMenu = ({ setShowMegaMenu, showmegaMenu }) => {
    ...
    return (
        <div onMouseLeave={(e) => { e.stopPropagation(), setShowMegaMenu(!showmegaMenu) }}
            ...
            <ul className="flex flex-1 flex-col justify-between h-full pb-8">
                ...
                {allContinents.map((continent, index) => (
                    <li
                        key={index}
                        className={`hover:bg-secondaryLight2 text-grayDark hover:text-secondaryDark p-4 capitalize flex items-center gap-2 ${selectedContinent === continent ? 'bg-secondaryLight2 text-secondaryDark' : ''
                            }`}
                    ...
                    {Array.from({ length: 12 }).map((_, index) => {
                        return (
                            <li key={index} className='p-4 text-[#5A5A5A] text-sm'>
                                <Link href="/continent">{selectedContinent}</Link>
                            </li>
                        );
                    })}
                </ul>

...
import { SingleProductAnimationForm } from "../Animation/Cards";
...
const bookingSchema = z.object({
    ...
    package: z.union([
        z.literal('scuba-diving'),
        z.literal('without-scuba-diving')
    ]).refine((val) => val !== undefined, { message: 'Please select a package' }),
                ...
                >
                    ...
                    <div className=" w-full flex flex-col gap-4">
                                    ...
                                    >
...

                                        {
                                            formData?.howMany?.adults !== undefined ? (
                                                <span>Adults {formData?.howMany?.adults} {formData?.howMany?.children > 0 ? (<span>Teen {formData?.howMany?.children}</span>) : ''}</span>
                                            ) : (
                                                <span>How Many?</span>
                                            )
                                        }
                        ...
                        <div className="flex rounded-lg absolute z-50 pointer-events-auto  top-[30%] w-full scale-90 sm:scale-[unset]">
                            {showCalendar && (
                                <div onMouseLeave={((e) => { setShowCalendar(!showCalendar) })} className="flex justify-center mx-auto bg-white w-fit rounded-2xl p-2">
                                    <Controller
                                        name="dateRange"
                                        control={control}
                                        render={({ field }) => (
                            ...
                            {showHowMany && (
                                <div className="text-nowrap flex flex-col gap-4  w-full items-center ">
                                    <div onMouseLeave={((e) => { setShowHowMany(!showHowMany) })} className="bg-white w-fit p-4 px-6 rounded-lg flex flex-col gap-4 border">
                    ...
                    <div className="w-full flex flex-col gap-4">
                        {errors.package && (
                            <div className="text-red-500 text-sm">{"Please Select Package"}</div>
                        )}
                        ...
                        {errors.withoutScubaDivingItem && (
                            <p className="text-red-500">{errors.withoutScubaDivingItem.message}</p>
                        )}
                        ...
                        <label htmlFor="scuba-diving" className="relative flex flex-col gap-4 bg-white py-4 rounded-xl text-Blueish font-semibold sm:text-lg cursor-pointer">
                            ...
                            {showScuvadiving && (
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="flex px-6 gap-4 bg-secondaryLight2 py-2">
                                        <div className="flex gap-2 font-normal items-center text-sm"><CircleCheckBig size={18} className=" accent-secondaryDark text-secondaryDark" /><span className="text-secondaryDark font-medium">Live Guide</span></div>
                                        <div className="flex gap-2 font-normal items-center text-sm"><Clock5 size={18} className="accent-secondaryDark text-secondaryDark" /><span className="text-secondaryDark font-medium">Duration - 1.5hrs</span></div>
                                    </div>
                            ...
                            <ChevronRight
                                className={`absolute top-0 right-0 mt-4 mr-4 ease-in-out duration-300 ${showScuvadiving ? 'rotate-90' : ''}`}
                            />
                        </label>

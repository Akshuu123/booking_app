...
const SingleProductCard = ({ productId, imgsrc, productRating, productTitle, productPrice, discount }) => {
    return (
        <div className={`${'product_' + productId} bg-white  rounded-lg p-4 gap-3 shadow-md sm:max-w-fit max-w-full min-h-[360px] h-fit w-full sm:mx-0`}>
            ...
            <div className='flex flex-col gap-[6px] justify-evenly py-1'>
                <div className='flex gap-1 text-secondaryDark text-sm'>
                    <Star className='fill-current' size={18} />{productRating || 4.5}<span className='text-[#5A5A5A]' dangerouslySetInnerHTML={{ __html: "(3.4K)" }} />
                </div>
                ...
                <div className='flex justify-between flex-wrap gap-2'>
                    <h5 className='flex flex-col  text-gray-500 font-semibold'><span className='font-normal'>From</span>{productPrice || "$1200"}</h5>
